...
template <int I>
__device__ fpcomplex plainBW_rio(fptype m12, fptype m13, fptype m23, unsigned int *indices) {
...
__device__ fpcomplex flatte_rio(fptype m12, fptype m13, fptype m23, unsigned int *indices) {
    // indices[1] is unused constant index, for consistency with other function types.
...
__device__ resonance_function_ptr ptr_to_RBW_rio      = plainBW_rio<1>;
__device__ resonance_function_ptr ptr_to_RBW_rio_SYM  = plainBW_rio<2>;
...
__device__ resonance_function_ptr ptr_to_FLATTE_Rio   = flatte_rio;
...
RBW_Rio::RBW_Rio(std::string name,
    ...
    if(symmDP) {
        ...
        GET_FUNCTION_ADDR(ptr_to_RBW_rio_SYM);

    } else {

        GET_FUNCTION_ADDR(ptr_to_RBW_rio);
...
FLATTE_Rio::FLATTE_Rio(std::string name,
    ...
    GET_FUNCTION_ADDR(ptr_to_FLATTE_Rio);
