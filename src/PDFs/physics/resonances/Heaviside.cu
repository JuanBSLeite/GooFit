#include <goofit/PDFs/physics/resonances/Heaviside.h>
...
__device__ fpcomplex cubicHeaviside(fptype m12, fptype m13, fptype m23, ParameterContainer &pc) {
    fpcomplex ret(0, 0);
    ...
    for(i = 0; i < timestorun; i++) {
        ...
        if(khiAB > 0 && khiAB < nKnobs) {
            ...
            fptype pwa_coefs_prime_real_kloAB = 1;
            fptype pwa_coefs_prime_real_khiAB = 1;
            fptype pwa_coefs_prime_imag_kloAB = 1;
            fptype pwa_coefs_prime_imag_khiAB = 1;
...
__device__ resonance_function_ptr ptr_to_Heaviside = cubicHeaviside;

namespace Resonances {

Heaviside::Heaviside(std::string name,
               Variable ar,
               Variable ai,
               ...
               unsigned int cyc,
               bool symmDP)
    : ResonancePdf("Heaviside", name, ar, ai) {
    ...
    registerFunction("ptr_to_Heaviside", ptr_to_Heaviside);

    recalculateCache();
}

__host__ void Heaviside::recalculateCache() const {
