...
#define MAXNKNOBS 1000
	...
	namespace Resonances {
		...
		class RBW : public ResonancePdf {
			public:
				RBW(std::string name,
						...
						unsigned int cyc,
						bool symmDP = false,
						bool ParentBachelorRestFrame=false);
		...
		class GS : public ResonancePdf {
			public:
				GS(std::string name, Variable ar, Variable ai, Variable mass, Variable width, unsigned int sp, unsigned int cyc,bool symDP = false, bool ParentBachelorRestFrame=false);
				~GS() override = default;
		};
